...
# forward
post_act1 = forward(x,params,name="layer1")
post_act2 = forward(post_act1,params,name="output",activation=softmax)
delta1 = post_act2 - y
# be sure to add loss and accuracy to epoch totals 
# backward
    ...
    if k.startswith("W"):
        for i in range(v.shape[0]):
            for j in range(v.shape[1]):
                ...
                post_act1 = forward(x,params,name="layer1")
                post_act2 = forward(post_act1,params,name="output",activation=softmax)
                loss,_ = compute_loss_and_acc(y,post_act2)
                grad_plus = loss
                #minus eps case
                params[k][i][j] = params_orig[k][i][j]-eps
                post_act1 = forward(x,params,name="layer1")
                ...
                grad_minus = loss
                #updating grad_v:
                grad_v[i][j]=(grad_plus-grad_minus)/(2*eps)
                params[k][i][j] = params_orig[k][i][j]
    else:
        for i in range(v.shape[0]):
            #plus eps case
            ...
            grad_minus = loss
            #updating grad_v:
            grad_v[i]=(grad_plus-grad_minus)/(2*eps)
            params[k][i] = params_orig[k][i]
